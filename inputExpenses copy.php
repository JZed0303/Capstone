...
function updateCurrentFund(value) {
    ...
    xhr.send();
                           ...
                           <input type="text"  class="form-control form-control-sm setBudget mt-2 px-1" id="amountInput" name="totalBudgetInput" value="" oninput="formatInput(this)">
...
// Get the first table with the id "dynamicTable"
// Function to calculate and update the total amount

function calculateTotalDynamic() {
  
  var total = 0;
  var amountCells = document.querySelectorAll(".amount-cell span.editable");
...
// Set the value of the td element to the input value
...
function updateHiddenInputs() {
    ...
    if (!errorFound) {
      for (var k = 0; k < expenseTableBody.rows.length; k++) {
        ...
        var dropdownCell = row.cells[2];
        var amountRow = row.cells[3]; // Assuming the dropdown is in the third cell of each row
        var selectedCategory = dropdownCell.querySelector('select').value;
        var selectedOption = dropdownCell.querySelector('select option:checked'); // Get the selected option
        var amountFinance = selectedOption.getAttribute('data-amount-finance'); // Get the data-amount-finance attribute

        var existingIndex = selectedData.findIndex(item => item.category === selectedCategory);

if (existingIndex !== -1) {
    // If it exists, update the amount
    selectedData[existingIndex].amount = amountFinance;
} else {
    // Otherwise, add a new object to the selectedData array
    selectedData.push({
...
for (var i = 0; i < selectedData.length; i++) {
    ...
    if (selectedFund === "Club Fund" || selectedFund === "") {
        currentRow += `<td class='validate text-center'>${selectedData[i].category}</td>`;
...
$(document).ready(function() {
  ...
  function getExpenseData() {
   ...
   var expenseParentCategory = document.getElementById("expenseParentCategory");
        ...
        // Reset the selected value of the fundCategory
        fundCategory.selectedIndex = 0;
...
xhr.send();
      ...
      // save amounts into dynamic and the table ouside  
            function saveAmount() {
                  ...
                  var formattedAmount=amountInput.value;
                     ...
                     amountInput.value = '';
                   if (isNaN(amount) || amount <= 0 || parseFloat(amount) > parseFloat(removeNonNumericCharacters(currentFundModal.textContent))) {
                    ...
                    selectedOptions.forEach(function(option) {
                      ...
                      if (!isOptionSelected) {
                        ...
                        var actionCell = row.insertCell(2); // No need to conditionally create this cell anymore
               amountCell.classList.add('amount-cell'); // Add the 'amount-cell' class to the amount cell
                        fundCell.textContent = optionText; // Populate the 'Fund' cell with the optionText
            
                  
                        amountCell.innerHTML = '<span class="editable" oninput="formatInput(this)" onclick="editAmount(this)">' + formattedAmount + '</span>';
      ...
      // edit dynamic tbale amounts
                  ...
                  deleteAllRowsFromExpense();
                  var editableSpan = cell.querySelector('.editable');
                  var currentAmount = parseFloat(removeNonNumericCharacters(editableSpan.textContent));
              ...
              // Apply styles to the input element to make it smaller
              editInput.style.width = '80px'; // You can adjust the width as needed
                  
                  // Function to handle updating the value and restoring the editable span
                  function updateValueAndRestoreSpan() {
  var newAmount = parseFloat(editInput.value);

  if (!isNaN(newAmount)) {
    // Update the amount in the 'selectedOptionsTable2'
    editableSpan.textContent = formatAmountWithCommas(newAmount);

    // Update the amount in the corresponding row of 'dynamicTable'
    var dynamicTable = document.getElementById('dynamicTable');
    var dynamicRow = dynamicTable.getElementsByTagName('tbody')[0].children[cell.parentNode.rowIndex - 1];
    dynamicRow.cells[1].textContent = formatAmountWithCommas(newAmount);    
  }


  // Replace the input element with the updated editable span
  cell.replaceChild(editableSpan, editInput);

  // Show the buttons again after editing
  showButtons(cell.parentNode);
    
  // Recalculate the total for both tables after editing
  calculateTotalDynamic();
  ...
  updateHiddenInputs();
                  ...
                  editInput.addEventListener('keydown', function (event) {
                    if (event.key === 'Enter') {
                      updateValueAndRestoreSpan();
                    }
                  });
            
                  // Replace the editable span with the input element
                  cell.replaceChild(editInput, editableSpan);
            
                  // Hide the buttons during editing
            ...
            function deleteRow(row) {
              deleteAllRowsFromExpense();
               updateHiddenInputs();
                ...
                var dynamicTable = document.getElementById('dynamicTable');
                var dynamicRow = dynamicTable.getElementsByTagName('tbody')[0].children[row.rowIndex - 1]; // The -1 is to get the corresponding row in dynamicTable
      ...
      // ----------------------------------END OF THE SCRIPT FOR SET BUDGET TABLE-----------------------------------------------------------------
   </script>
...
function balanceFinance() {
  let totalBalance = 0; // Initialize totalBalance variable
  
  document.getElementById("expenseTableContainer").addEventListener("change", function(event) {
    var target = event.target;
    if (target.tagName === "SELECT") {
      var allRows = document.querySelectorAll("tr");
      var selectedOption = target.value;
      var totalAmountForSelectedOption = 0;
      var selectedAmount = optionsData.find(item => item.category === selectedOption).amount;
      var expenseAmount = document.getElementsByClassName('expenseAmount');

      allRows.forEach(function(row) {
        var select = row.querySelector("select");
        if (select && select.value === selectedOption) {
          var amountCell = row.cells[1];
          var amountCellValue = amountCell.textContent;
          var amountInSecondColumn = parseFloat(removeNonNumericCharacters(amountCellValue));
          totalAmountForSelectedOption += amountInSecondColumn;
        }
      });

      if (selectedAmount < totalAmountForSelectedOption) {
        var expenseAmountInput = document.querySelector('td.expenseAmount');
        exceed= true;
        expenseAmountInput.textContent = 0;
        balanceElement.textContent = 0;
        return;
      } else {
        var newBalance = selectedAmount - totalAmountForSelectedOption;
        // Update the balance element for the specific row
        var balanceElement = target.closest("tr").querySelector('.input-column');
        balanceElement.textContent = formatAmountWithCommas(newBalance);
      }
  ...
  if(exceed){
    showErrorModal("Error: Please Check your set budget");
    return;
  }
}
...
// Create a function to populate the dropdown options based on data
function populateDropdownOptions(selectElement, optionsData) {
    ...
    for (var i = 0; i < optionsData.length; i++) {
        ...
        option.textContent = optionsData[i].category; // Display category
        option.setAttribute('data-amount-finance', optionsData[i].amount); // Set the data-amount-finance attribute
...
newRow.innerHTML = `
  <td class="title-cell" style=" white-space: nowrap;"class="text-center d-flex" >${expenseTitle}</td>
  <td class="text-right expenseAmount">${formatAmountWithCommas(expenseAmount)}</td>
  ...
  ${
    selectedFundCategory === 'Club Fund'
      ? `<td class="text-right expenseThrough" id="balanceInput"><span class="input-column" id="inputColumn" data-total-budget="${totalBudgetInput}"></span></td>`
      : `<td class="text-center">${formatAmountWithCommas(balanceNotClubFund)}</td>`
  }   
  <td class="text-center d-flex justify-content-center g-3">
    <button type="button" class="expenses-edit mx-1 btn btn-sm btn-primary btn-edit">Edit</button>
...
//----------------------------------------FUNCTION TO CALCULATE THE TOTAL AMOUNT IN EXPENSE---------------------------------------
      ...
      deleteButton.addEventListener("click", function() {
      deleteExpenseItem(row);
      });
      var amountCell = row.querySelector("td:nth-child(2)");
      amountCell.addEventListener("click", function() {
      editExpenseItem(row);
      });
            //---------------------------------------END buttondS FOR DYNAMIC TABLE---------------------------------------

      
      }
      //----------------------------------------FUNCTION TO UPDATE OR EDIT THE AMOUNT IN EXPENSE---------------------------------------
      function editExpenseItem(row) {


        
      // Get the current expense amount from the row
      var currentAmount = parseFloat(row.querySelector("td:nth-child(2)").textContent.replace(/,/g, ""));
      var totalBudgetInput = document.getElementById('totalBudgetInput');
      // Check if the row is already being edited (to prevent re-editing while editing)
      ...
      // Store the current cell content in a variable
      var amountCell = row.querySelector("td:nth-child(2)");
      var originalContent = parseFloat(removeNonNumericCharacters(amountCell.textContent));
      
      // Create an input field for editing the amount
      var amountInput = document.createElement("input");
      amountInput.type = "number";
      amountInput.step = "0.01";
      amountInput.value = currentAmount;
      amountInput.autofocus = true; // Set autofocus to the input field
      ...
      // Add a click event listener to the input field to stop event propagation
      amountInput.addEventListener("click", function (event) {
      ...
      // Get the updated amount from the input field
      var updatedAmount = parseFloat(amountInput.value);
      
      editButton.style.display = "inline-block";
      
      // Get the total budget input
      var totalBudgetInput = document.getElementById("totalBudgetInput");
      var totalAmountFormatted = totalBudgetInput.value;
      
   var expenseAmountElements = document.querySelectorAll('td.text-right.expenseAmount');
      var totalAmount = 0;
      expenseAmountElements.forEach(function(element) {
      var expenseAmount = parseFloat(removeNonNumericCharacters(element.textContent));
      ...
      totalExpenseAmount = parseFloat(removeNonNumericCharacters(totalAmountElement.textContent));

    
       var newTotal = totalExpenseAmount + updatedAmount;
       var budget = parseFloat(removeNonNumericCharacters(totalAmountFormatted));
  
       if (  parseFloat(newTotal)>budget){
         showErrorModal("Expense amount cannot exceed the total");
           updatedAmount = originalContent;
         return;
       
      }
      // Check if the updated amount is valid (a number) and does not exceed the total budget
      if (!isNaN(updatedAmount) && updatedAmount <= parseFloat(removeNonNumericCharacters(totalAmountFormatted))) {
                // Update the cell content with the new amount
      ...
      // Remove the "editing" class to allow future editing
      row.classList.remove("editing");
      }
      }
      
      document.addEventListener("keydown", handleEnterKey);
      var inputElement = row.querySelector('.input-column'); // Replace with the actual input element selector for this row
    var dropdownElement = row.querySelector('.dropdown.finance-through.btn.btn-primary'); // Replace with the actual dropdown element selector for this row  
...
function calculateBalanceAmount(){
   // Assuming you have an element with an ID of "inputColumn"
   var balanceAmountElements = document.querySelectorAll("#balanceInput");

balanceAmountElements.forEach(function(element) {
    var spanElement = element.querySelector('.input-column'); // Find the <span> element inside each <td>
    console.log(spanElement);
    // Extract the numeric value from the text content var numericValue = textContent.match(/₱(\d+\.\d+)/);
      ...
      // Get the expense table container
      var expenseTableContainer = document.getElementById("expenseTableContainer");
      
      // Check if the total amount is zero (no expenses)
      if (totalAmount === 0) {
