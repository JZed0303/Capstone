...
if (isset($_SESSION['success_message'])) {
    ...
    unset($_SESSION['success_message']); // Clear the message to avoid displaying it again on page refresh
}
                              ...
                              <div class="form-row pl-md-3 mt-md-4 pl-lg-3 mt-lg-4 ">
                                 ...
                                 <div class="form-group col-12 col-md-8 col-lg-8 p-0" id="clubFundTbl" style="display:none;">
                                    <div class="float-right p-2">
                                       <span id="edit-table" class="btn btn-sm btn-success float-right w-"><i class="fa fa-pencil"></i></span>
                                    </div>
                                    =
                              ...
                              <div class="form- mt-4  ">
                                    ...
                                    <div class="TotalCurrentNotClubFund d-none">
                                     <span style=" color: #555;white-space:nowrap;" class="notClubFund d-flex flow-row">Total Current Fund: &nbsp; <strong><span class="ms-3" style="border-bottom:blue 1px solid;">
                                       <p style="border-bottom:blue 1px solid;" id="notClubFundValue" class="notClubFundValue"></p>
                                       <strong class="text-secondary">
                                          <span id="currentFund">
                                       ...
                                       <div class="form-group " style="width: 100%;">
                                          <div class="d-flex flex-column  flex-md-row flex-lg-row ">
                                             ...
                                             <div class="col-sm-8 mt-2  col-md-6 col-lg-6 d-flex  justify-content-center align-items-center">
                                                ...
                                                <input type="text" required="" style="width:60%;" class="form-control form-control-sm expense-amount mt-2" name="expense_amount[]" id="expenseAmountInput" value="" oninput="formatInputExpense(this)" placeholder="0">
                                              </div>
...
<div class="col-12 d-flex justify-content-between">
<p>Date:</p> <p class="me-3"> ${location}</p>
</div>
</div>
<hr>
...
function validateForm() {
  ...
  if (!errorFound) {
    ...
    // Check if any row in column 2 (third column) contains a zero value
    let hasZeroInColumn2 = false;
    for (let i = 0; i < rows.length; i++) {
      const secondColumn = rows[i].getElementsByTagName("td")[1]; // Get the third column

      const currentContent = parseFloat(removeNonNumericCharacters(secondColumn.textContent.trim()));

      if (currentContent === 0) {
        hasZeroInColumn2 = true;
        break;
      }
    }

    if (hasZeroInColumn2) {
      showErrorModal("Expense cannot be Zero");
      return;
    }
...
// Check the initial value of the select element
// if (selectElement.value === "no_data") {
//     expenseInput.disabled = true;
//     expenseInput.value = ""; // Clear the input when disabled if needed
// }
...
// Store the original value of the total budget in the hidden input
 ...
 <script>
  ...
  var maxBudgetElement = document.getElementById('notClubFundValue').textContent;
  var maxBudget = parseFloat(removeNonNumericCharacters(maxBudgetElement));
  var value = removeNonNumericCharacters(input.value); // Remove non-numeric characters from the input
  var amount = parseFloat(value); // Convert the input value to a number

  if (!isNaN(amount) || value === '') {
    // Check if the entered amount is greater than the maxBudget
    if (amount <= maxBudget) {
      var formattedAmount = formatAmountWithCommas(amount);
      input.value = formattedAmount; // Set the formatted value back to the input field
    } else {
      // If the amount exceeds maxBudget, set the input value to the maxBudget
      input.value = formatAmountWithCommas(maxBudget);
    }
  } else {
    input.value = ""; // Set the input value without formatting (non-numeric input)
  }
...
// Get the selected value from the 'fundCategory' elem
...
document.getElementById("expenseTableContainer").addEventListener("change", function(event) {
  ...
  if (target.tagName === "SELECT") {
      ...
      if(selectedAmount<totalAmountForSelectedOption){
          // showErrorModal("Cannot have negative");
          return;
      }else{
...
${
  selectedFundCategory === 'Club Fund'
    ? `<td class="text-right expenseThrough"><span class="input-column" id="inputColumn" data-total-budget="${item.balance}"></span></td>`
    : `<td class="text-center id="balance" ">${formatAmountWithCommas(item.balance)}</td>`
}  
          <td class="text-center">
              <button type="button" class="expenses-edit btn btn-sm btn-primary btn-edit">Edit</button>
              <button type="button" class="expenses-delete btn btn-sm btn-danger btn-delete">Delete</button>
