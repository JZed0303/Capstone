...
var errorFound = false; // Flag to track if an error is found
  ...
  var totalExpenseAmount = 0; // Variable to store the total expense amount

  // Get the selected data from the dynamicTable
  ...
  for (var j = 0; j < dynamicTableBody.rows.length; j++) {
    ...
    var existingIndex = selectedData.findIndex(item => item.category === category);
    if (existingIndex !== -1) {
        selectedData[existingIndex].amount = amount; // Update amount if category exists
    } else {
        selectedData.push({
            category: category,
            amount: amount,
...
if(selectedFund == "Club Fund"){
   var dropdownCell = row.cells[2]; // Assuming the dropdown is in the third cell of each row
        ...
        var selectedCategory = dropdownCell.querySelector('select').value; // Get the selected category from the dropdown value
        ...
        if (existingIndex !== -1) {
            selectedExpenseData[existingIndex].amount = selectedAmount; // Update amount if category exists
     ...
     selectedExpenseData.push({
      ...
      financeThrough: selectedAmount, // Add financeThrough amount
    });
// for club fund
  }   
} else{
...
console.log(selectedExpenseData);
  ...
  // Update the hidden input field with the total expense amount
